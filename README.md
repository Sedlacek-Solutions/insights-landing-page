...
- The feature section uses one sticky screenshot area with animated CSS spotlight crops as each text step scrolls into focus. Metrics and request-removal steps swap screenshots via `data-image`. Adjust `data-spot-x`, `data-spot-y`, `data-spot-w`, and `data-spot-h` on each `.feature-step` in `index.html` if a highlight needs to move.
